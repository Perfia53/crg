...
class RunDayCommand extends Command
{
    ...
    {
        $this->setName('app:runDay');
    }

    protected function execute(InputInterface $input, OutputInterface $output)
    {
        $sToday = date("ymd");
        $log = fopen('nette.safe://'.__DIR__ . '/../../log/cron/runDay' . $sToday . '.log', 'a+'); // před jméno souboru dáme nette.safe://
        fwrite($log,CHR(13).CHR(10).'Start script - '.date("d.m.y H:i:s").CHR(13).CHR(10));

        /*..obsah denniho bloku*/

        fwrite($log,'Run day finished - '.date("d.m.y H:i:s").CHR(13).CHR(10));
